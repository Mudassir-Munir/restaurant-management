...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		startIndex := (page - 1) * recordsPerPage // skip and limit: explore
		...
		c.JSON(http.StatusOK, allFoods[0]) // Do we need index here?
...
func GetFood() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		err := foodCollection.FindOne(context, bson.M{"food_id": foodId}).Decode(&food)
		defer cancel()

		if err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while fetching the food by id"})
		}
...
func CreateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// menu in which food item will be inserted
		err := menuCollection.FindOne(ctx, bson.M{"munu_id": food.Menu_id}).Decode(&menu)
		defer cancel()
		if err != nil {
		...
		food.Created_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339)) // check here
...
func UpdateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprintf("update food item failed")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
